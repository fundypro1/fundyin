        ...
        <div class="form-container">
          ...
          <form @submit.prevent="handleSignup" class="login-form">
            ...
            <div class="input-group">
              ...
              <small v-if="validationErrors.phone" class="field-error">{{ validationErrors.phone }}</small>
            </div>
  ...
  methods: {
    async handleSignup() {
      ...
      try {
        ...
        setTimeout(() => this.$router.push('/login?registered=true'), 2000)
      } catch (err) {
    ...
    validatePhone() {
      if (this.user.phone && this.user.phone.trim()) {
        ...
        const cleanPhone = this.user.phone.replace(/[\s\-\(\)]/g, '')
